...
def collect_register_offsets():
    # (registry, offset) -> {"per_model": {model_filename: set[(size, conv_id)]}}
    index: dict[tuple[int, int], dict[str, dict[str, set]]] = {}
    ...
    for path in sorted(DEF_DIR.glob("*.h")):
        ...
        for line in path.read_text(encoding="utf-8", errors="ignore").splitlines():
            ...
            if len(parts) < 4:
                continue
            reg_str, off_str, conv_str, size_str = parts[:4]
            try:
                reg = parse_int(reg_str)
                off = parse_int(off_str)
                conv = parse_int(conv_str)
                size = parse_int(size_str)
            except ValueError:
                continue
            key = (reg, off)
            entry = index.setdefault(key, {"per_model": {}})
            per_model = entry["per_model"].setdefault(model_name, set())
            per_model.add((size, conv))
    ...
    for (reg, off) in sorted(index.keys(), key=lambda ro: (ro[0], ro[1])):
        ...
        sorted_pairs.append(((reg, off), per_model, models))
...
def write_markdown(pairs_with_models, ordered_model_filenames):
    """Write a matrix of (registry, offset) vs models, with per-model size/conv.

    pairs_with_models: [((reg, off), per_model_map, [model_filename, ...]), ...]
      where per_model_map is {model_filename: set[(size, conv_id)]}.
    ...
    for (reg, off), per_model, models in pairs_with_models:
        ...
        for fn in ordered_model_filenames:
            if fn not in present:
                row.append("")
                continue
            entries = sorted(per_model.get(fn, set()))  # [(size, conv), ...]
            # Format as "X (size/conv; ...)" for visibility
            parts = [f"{size}/{conv}" for (size, conv) in entries]
            cell = "X (" + "; ".join(parts) + ")" if parts else "X"
            row.append(cell)
